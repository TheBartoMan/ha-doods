...
class _CameraStepsMixin:
    ...
    _existing_label_confidences: dict[str, float]
    ...
    async def async_step_manage(
        ...
        return self.async_show_menu(step_id="manage", menu_options=menu_options)  # type: ignore[attr-defined]
    ...
    async def async_step_add_camera(
        ...
        """
        # If we're editing, self._camera_draft/_existing_label_confidences
        ...
        schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_LABELS,
                    default=list(self._existing_label_confidences),
                ): SelectSelector(
                    SelectSelectorConfig(
    ...
    async def async_step_label_confidence(
        ...
        """Ask for a confidence override, one selected label at a time."""
        if user_input is not None and self._label_queue:
            label = self._label_queue.pop(0)
            self._camera_draft[CONF_LABELS][label] = user_input[CONF_CONFIDENCE]

        if self._label_queue:
            label = self._label_queue[0]
            default_confidence = self._existing_label_confidences.get(
                label, self._base_confidence
            )
            schema = vol.Schema(
                {
                    vol.Required(
    ...
    async def async_step_area(
        ...
        if user_input is not None:
            ...
            self._existing_label_confidences = {}
    ...
    async def async_step_edit_camera(
        ...
        if user_input is not None:
            ...
            self._existing_label_confidences = dict(camera.get(CONF_LABELS) or {})
...
class DoodsConfigFlow(ConfigFlow, _CameraStepsMixin, domain=DOMAIN):
    ...
    def __init__(self) -> None:
        ...
        self._existing_label_confidences: dict[str, float] = {}
    ...
    async def async_step_import(
        ...
        for label in import_config.get(CONF_LABELS, []):
            if isinstance(label, str):
                label_confidences[label] = base_confidence
            else:
                label_confidences[label[CONF_NAME]] = label.get(
